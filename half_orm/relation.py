...
from half_orm.op_tree import OpTree
...
def ho_is_set(self):
    ...
    return (joined_to or bool(self.__op_tree.operator) or bool(self.__neg) or
...
#@utils.trace
def __walk_op_tree(self, rel_id_=None, out=None, _fields_=None):
    """Walks the set operators tree and return a list of SQL where
    ...
    if out is None:
        out = []
        _fields_ = []
    if self.__op_tree.operator:
        if self.__neg:
            out.append("not (")
        out.append("(")
        left = self.__op_tree.left
        left.__query_type = self.__query_type
        left.__walk_op_tree(rel_id_, out, _fields_)
        if self.__op_tree.right is not None:
            out.append(f" {self.__op_tree.operator}\n    ")
            right = self.__op_tree.right
            right.__query_type = self.__query_type
            right.__walk_op_tree(rel_id_, out, _fields_)
        out.append(")")
        if self.__neg:
            out.append(")")
    else:
...
def __where_args(self, *args):
    ...
    what = f'r{self.ho_id}.*'
    if args:
        what = ', '.join([f'r{self.ho_id}.{arg}' for arg in args])
    s_where, set_fields = self.__walk_op_tree()
...
def __set__op__(self, operator=None, right=None):
    ...
    new.__op_tree.set(self, operator, right)
...
def __neg__(self):
    new = self.__set__op__(self.__op_tree.operator, self.__op_tree.right)
