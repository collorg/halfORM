...
class Test(TestCase):
    ...
    def not_set_field_test(self):
        pers = self.pers()
        fields_set = {elt.is_set() for elt in self.pers.fields.values()}
        self.assertTrue(fields_set, {False})

    def set_field_test(self):
        pers = self.pers(first_name='jojo')
        self.assertTrue(pers.fields.first_name.is_set())

    def idem_test(self):
        pers = self.pers(first_name='jojo')
        self.assertEqual(isinstance(pers.fields.first_name, Field), True)

    def fields_names_test(self):
        field_names = {elt.name() for elt in self.pers.fields.values()}
        print(field_names)
        self.assertEqual(
            field_names,
    ...
    def relation_ref_test(self):
        first_name = self.pers.fields.first_name
